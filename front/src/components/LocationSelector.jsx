...
export default function LocationSelector({ onSelect, initialCountry = 'MX', initialState = '' }) {
    ...
    fetch('/countries.json')
      ...
      .then(data => {
        ...
        if (defaultCountry) {
          ...
          if (initialState) {
            const defaultState = defaultCountry.states.find(s => s.code === initialState || s.name === initialState);
  ...
  const filteredItems = mode === 'country' 
    ? countries.filter(c => c.name.toLowerCase().includes(searchQuery.toLowerCase()))
    : selectedCountry?.states.filter(s => s.name.toLowerCase().includes(searchQuery.toLowerCase())) || [];
  ...
  const handleSelect = (item) => {
    if (mode === 'country') {
      ...
      onSelect({ country: selectedCountry.code2, state: item.code || item.name });
      setIsOpen(false);
    }
  };
  ...
  if (loading) return <div className="animate-pulse h-12 bg-zinc-800 rounded-xl"></div>;
        ...
        <div className="absolute z-50 mt-2 w-full bg-zinc-900 border border-zinc-800 rounded-xl shadow-xl overflow-hidden">
          ...
          <div className="max-h-60 overflow-y-auto">
            {mode === 'state' && (
              <button
                type="button"
                onClick={() => { setMode('country'); setSearchQuery(''); }}
            ...
            {filteredItems.length === 0 ? (
              ...
              filteredItems.map((item) => (
                ...
                >
                  ...
                  {(selectedCountry?.code2 === item.code2 || selectedState?.code === item.code) && (
