...
| language | String | ❌ | size 3 | — | Si quieres `es`, setéalo en app. *(O usa es-MX y sube el tamaño si lo ocupas.)* |
...
| Installments | Integer | ✅ | min 1 | — | Nota: te recomiendo renombrar a `installments` (minúsculas). |
...
## Relaciones
- `profile` → `users_info` (**many-to-one**, `cascade`)
- `transaction` → `transactions` (**many-to-one**, `cascade`) *(según tu lista)*
