...
export default function Profile() {
  ...
  useEffect(() => {
    if (userInfo) {
      ...
      if (userInfo.avatarFileId) {
        const url = `https://appwrite.racoondevs.com/v1/storage/buckets/${import.meta.env.VITE_APPWRITE_AVATARS_BUCKET_ID}/files/${userInfo.avatarFileId}/view?project=${import.meta.env.VITE_APPWRITE_PROJECT_ID}`;
        setAvatarUrl(url);
      }
    }
  }, [userInfo]);
  ...
  const handleCropComplete = async (croppedBlob) => {
    ...
    try {
      ...
      const url = `https://appwrite.racoondevs.com/v1/storage/buckets/${import.meta.env.VITE_APPWRITE_AVATARS_BUCKET_ID}/files/${response.$id}/view?project=${import.meta.env.VITE_APPWRITE_PROJECT_ID}`;
      setAvatarUrl(url);
    ...
    mutationFn: async (data) => {
      ...
      if (fullName && fullName !== user.name) {
        try {
          ...
          // Don't fail the whole mutation if this fails, but maybe warn?
        }
      }
    ...
    <div className="p-6 max-w-2xl mx-auto mb-20 md:mb-0">
      ...
      {!user.emailVerification && (
        <div className="mb-6 bg-yellow-500/10 border border-yellow-500/20 rounded-xl p-4 flex items-start gap-3">
          ...
          <div className="flex-1">
            <h3 className="text-yellow-500 font-medium mb-1">Email not verified</h3>
            ...
            {!verificationSent ? (
              <Button size="sm" variant="secondary" onClick={handleSendVerification} className="bg-yellow-500/20 text-yellow-500 hover:bg-yellow-500/30 border-transparent">
                <Mail size={16} className="mr-2" />
                Send Verification Email
              </Button>
            ) : (
              <p className="text-sm text-emerald-500 font-medium">Verification email sent! Please check your inbox.</p>
            )}
          </div>
        </div>
      )}
      ...
      <div className="bg-zinc-900 border border-zinc-800 rounded-2xl p-6">
        <div className="flex flex-col items-center mb-8">
          ...
          <p className="mt-4 text-zinc-400 text-sm">Click camera icon to upload new photo</p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <LocationSelector
            ...
            onSelect={(loc) => setFormData({ ...formData, country: loc.country, state: loc.state })}
          />
          ...
          <div className="pt-4 border-t border-zinc-800 flex justify-end">
            <Button type="submit" disabled={updateProfileMutation.isPending || uploadingAvatar}>
